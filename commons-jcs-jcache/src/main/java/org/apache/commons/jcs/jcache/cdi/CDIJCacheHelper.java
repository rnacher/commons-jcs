...
public class CDIJCacheHelper {
	...
	// it is unlikely we have all annotations but for now we have a single meta
	// model
	private MethodMeta createMeta(final InvocationContext ic) {
		final CacheDefaults defaults = findDefaults(ic.getTarget() == null ? null : ic.getTarget().getClass(),
				ic.getMethod());
		...
		final String cachePutCachePutName = cachePut == null ? null
				: defaultName(ic.getMethod(), defaults, cachePut.cacheName());
		...
		return new MethodMeta(parameterTypes, annotations, mtdAnnotations, keyParameterIndexes(ic.getMethod()),
				...
				cachePut, cacheRemoveCacheRemoveName, cacheRemoveCacheResolverFactory, cacheRemoveCacheKeyGenerator,
				cacheRemove != null && cacheRemove.afterInvocation(), cacheRemove, cacheRemoveAllCacheRemoveAllName,
				cacheRemoveAllCacheResolverFactory, cacheRemoveAll != null && cacheRemoveAll.afterInvocation(),
				cacheRemoveAll);
	}
	...
	private <T> T instance(final Class<T> type) {
		...
		try {
			...
			if (normalScope) { // TODO: release at the right moment, @PreDestroy? question is: do we assume it
								// is thread safe?
				context.release();
			}
		}
	...
	public static class MethodMeta {
		...
		public MethodMeta(Class<?>[] parameterTypes, List<Set<Annotation>> parameterAnnotations,
				Set<Annotation> annotations, Integer[] keysIndices, Integer valueIndex, Integer[] parameterIndices,
				String cacheResultCacheName, CacheResolverFactory cacheResultResolverFactory,
				CacheKeyGenerator cacheResultKeyGenerator, CacheResult cacheResult, String cachePutCacheName,
				CacheResolverFactory cachePutResolverFactory, CacheKeyGenerator cachePutKeyGenerator,
				boolean cachePutAfter, CachePut cachePut, String cacheRemoveCacheName,
				...
				CacheResolverFactory cacheRemoveAllResolverFactory, boolean cacheRemoveAllAfter,
